...
static void app_main_ws_text_cb(const char *data, int len)
{
    ...
    else if (strcmp(type_str, "iot") == 0)
    {
        ...
        for (int i = 0; i < commands_size; i++)
        {
            ...
            else if (strcmp(method_str, "TurnOn") == 0)
            {
            ...
            else if (strcmp(method_str, "TurnOff") == 0)
            {
                // 关灯
                Int_WS2812_LEDClose();
            }
    ...
    {
        MY_LOGE("receive llm data : %.*s", len, data);

        // 提取 emotion
        cJSON *emotion = cJSON_GetObjectItem(root, "emotion");
        char *emotion_str = cJSON_GetStringValue(emotion);
        App_display_ShowEmoji(emotion_str);
...
static void app_main_ws_finish_cb(void)
{
    ...
    // lcs显示默认
    App_display_ShowText("            小智AI聊天机器人");
    App_display_ShowEmoji("cool");
...
void app_main(void)
{
    ...
    Dri_nvs_Init();
    Dri_WIFI_Init();	// 先启动WIFI,配网结束释放资源
    Int_WS2812_Init();
    Int_lcd_Init();
    App_display_Init();
    Int_ES8311_Init();
